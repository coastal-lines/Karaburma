...
TABLE_CELLS_COLOUR = (255, 128, 128)
...
def draw_listbox(screenshot_copy_debug, screenshot_elements):
    for listbox in (item for item in screenshot_elements.get_elements() if item.get_label() == "listbox"):
        ...
        if (listbox.get_v_scroll() != None):
            draw_custom_elements(screenshot_copy_debug, [listbox.get_v_scroll()], "v_scroll", LISTBOX_COLOUR,
                                 TABLE_SCROLLS_THICKNES)
            draw_custom_elements(screenshot_copy_debug, [listbox.get_v_scroll().get_first_button()], "UP",
                                 LISTBOX_COLOUR, TABLE_SCROLLS_THICKNES)
            draw_custom_elements(screenshot_copy_debug, [listbox.get_v_scroll().get_second_button()], "DOWN",
                                 LISTBOX_COLOUR, TABLE_SCROLLS_THICKNES)

def draw_tables(screenshot_copy_debug, screenshot_elements):
    for table in screenshot_elements.get_table_elements():
        draw_custom_elements(screenshot_copy_debug, screenshot_elements.get_elements(), "table", TABLE_COLOUR,
                             TABLE_THICKNES)

        if (table.get_v_scroll() != None):
            draw_custom_elements(screenshot_copy_debug, [table.get_v_scroll()], "v_scroll", TABLE_SCROLLS_COLOUR,
                                 TABLE_SCROLLS_THICKNES)
            draw_custom_elements(screenshot_copy_debug, [table.get_v_scroll().get_first_button()], "UP",
                                 TABLE_SCROLLS_COLOUR, TABLE_SCROLLS_THICKNES)
            draw_custom_elements(screenshot_copy_debug, [table.get_v_scroll().get_second_button()], "DOWN",
                                 TABLE_SCROLLS_COLOUR, TABLE_SCROLLS_THICKNES)

        if (table.get_h_scroll() != None):
            draw_custom_elements(screenshot_copy_debug, [table.get_h_scroll()], "h_scroll", TABLE_SCROLLS_COLOUR,
                                 TABLE_SCROLLS_THICKNES)
            draw_custom_elements(screenshot_copy_debug, [table.get_h_scroll().get_first_button()], "LEFT",
                                 TABLE_SCROLLS_COLOUR, TABLE_SCROLLS_THICKNES)
            draw_custom_elements(screenshot_copy_debug, [table.get_h_scroll().get_second_button()], "RIGHT",
                                 TABLE_SCROLLS_COLOUR, TABLE_SCROLLS_THICKNES)
...
def draw_elements(screenshot_copy_debug, screenshot_elements) -> np.ndarray:
    ...
    draw_custom_elements(screenshot_copy_debug, screenshot_elements.get_elements(), "button", BUTTONS_COLOUR,
                         BUTTONS_THICKNES)
    draw_custom_elements(screenshot_copy_debug, screenshot_elements.get_elements(), "input", INPUT_COLOUR,
                         INPUT_THICKNES)
    draw_custom_elements(screenshot_copy_debug, screenshot_elements.get_elements(), "radiobutton", RADIOBUTTON_COLOUR,
                         RADIOBUTTON_THICKNES)
    draw_custom_elements(screenshot_copy_debug, screenshot_elements.get_elements(), "combobox", COMBOBOX_COLOUR,
                         COMBOBOX_THICKNES)
    draw_custom_elements(screenshot_copy_debug, screenshot_elements.get_elements(), "checkbox", CHECKBOX_COLOUR,
                         CHECKBOX_THICKNES)
    draw_custom_elements(screenshot_copy_debug, screenshot_elements.get_elements(), "slider", SLIDER_COLOUR,
                         SLIDER_THICKNES)
