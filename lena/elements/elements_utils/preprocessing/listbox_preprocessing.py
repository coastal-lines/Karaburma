    ...
    def get_contours_for_listbox(self, image):
        ...
        for i in range(len(contours)):
            ...
            if (min_w < w < max_w and min_h < h < max_h):
                #DEBUG -изучить больше про контуры!!!!
                parent_index = hierarchy[0][i][3]
                #print(parent_index)

                #родительским контуром может оказаться глобальный контур, т.е. его можно исключить
                #проверяем родителя даже если его родительский индекс не равен "-1"
                if(parent_index != -1):
        ...
        #DEBUG - скорее всего не нужно фильтровать. очищено за счет контуров
