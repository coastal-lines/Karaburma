...
class TablePreprocessing:
    ...
    def __table_image_processing(self, roi):
        ...
        #cv2.imwrite("f:\\Data\\Work\\OwnProjects\\Python\\Demo_Screens\\test6" + "\\" + f, otsu_binary)
        ...
        num_clusters = 16
        # random_state=102 - можно проговорить что это важно. Например для хариса и таблиц оказалось критично. Находилось то меньше то больше точек.
        ...
        try:
            ...
            #cv2.imwrite("f:\\Data\\Work\\OwnProjects\\Python\\Demo_Screens\\test6" + "\\" + image_path.split("\\")[-1], otsu_binary)
        ...
        if(len(result_rectangles) > 0):
            ...
            if(len(super_result_rectangles) > 0):
                ...
                for i in range(len(super_result_rectangles)):
                    ...
                    #cv2.imwrite(r"f:\Data\Work\OwnProjects\Python\BigPagesForTrainingNon\temp_for_contours" + "\\" + screen.path + str(generate_random_string(9)) + ".png", cv2.cvtColor(temp_image, cv2.COLOR_BGR2RGB))
