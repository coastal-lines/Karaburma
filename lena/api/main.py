...
class LenaApiService:
    def __init__(self, host, port, config_path, source_mode, detection_mode, logging):

        self.__server = None

        self.__lena_instance = Lena(config_path, source_mode, detection_mode, logging)
        self.__app = FastAPI()
        ...
        def get_all_elements(img_path):
            return {"message": self.__lena_instance.find_all_elements(img_path)}
